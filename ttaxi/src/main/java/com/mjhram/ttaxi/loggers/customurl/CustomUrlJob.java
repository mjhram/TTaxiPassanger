package com.mjhram.ttaxi.loggers.customurl;

import android.location.Location;

import com.mjhram.ttaxi.common.MyInfo;
import com.mjhram.ttaxi.common.SerializableLocation;
...
public class CustomUrlJob extends Job {
    ...
    public CustomUrlJob(String customLoggingUrl, MyInfo info, String annotation, int satellites, float batteryLevel, String androidId) {
        super(new Params(1).requireNetwork().persist());
        info.update();
        this.loc = new SerializableLocation(info);
        //this.annotation = annotation;
        this.satellites = satellites;
        this.logUrl = customLoggingUrl;
        this.batteryLevel = batteryLevel;
    ...
    public void onRun() throws Throwable {
        ...
        // &spd=%SPD&time=%TIME&battery=%BATT&androidId=%AID&serial=%SER&state=%st";
        if(loc.updateStateOnly == false) {
            ...
            logUrl = logUrl.replaceAll("(?i)%st", String.valueOf(loc.availState));
        } else {
            logUrl = "http://www.ttaxi1.com/logTaxiLoc.php?tag=updateState&email=mj@a.com&state=%ST";
            logUrl = logUrl.replaceAll("(?i)%st", String.valueOf(loc.availState));
        }
