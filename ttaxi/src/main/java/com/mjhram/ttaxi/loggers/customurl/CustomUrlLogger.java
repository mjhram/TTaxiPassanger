...
import com.mjhram.ttaxi.loggers.IFileLogger;
...
public class CustomUrlLogger implements IFileLogger {
    ...
    @Override
    public void Write(MyInfo info) throws Exception {
        if (!Session.hasDescription()) {
            Annotate("", info);
        }
    }
    ...
    @Override
    public void Annotate(String description, MyInfo info) throws Exception {
        JobManager jobManager = AppSettings.GetJobManager();
        jobManager.addJobInBackground(new CustomUrlJob(customLoggingUrl, info, description, satellites, batteryLevel, androidId));
    }
